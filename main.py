...
def leave_one_out_cross_validation(data,set_of_features,feature):
    ...
    for j in range(1,len(test_dataset.columns)):
        if j not in set_of_features and j != feature:
            test_dataset =test_dataset.drop(columns=[j])
    ...
    for i in range(len(test_dataset)):
        object_to_classify = test_dataset.iloc[i,1:]
        label_object_to_classify = test_dataset.at[i,0]
        ...
        for k in range(len(test_dataset.columns)):
            if k != i:
                distance = math.sqrt(sum(pow((object_to_classify - test_dataset.iloc[k,1:]),2)))
                if distance < nearest_neighbor_distance:
                    nearest_neighbor_distance = distance
                    nearest_neighbor_location = k
                    nearest_neighbor_label = test_dataset.at[nearest_neighbor_location,0]
    ...
    return (number_correctly_classfied / (len(test_dataset)))
...
#test_set = set()
#test_set.add(2)
#num = leave_one_out_cross_validation(main_data,test_set,4)
#print(num)